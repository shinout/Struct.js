const Modifiers = {
  ...
  integer: function(v) {
    ...
    if (typeof v != 'number') {
      if (strict) {
        throw this.error({reason: 'not number', val: v});
      }
      else {
        var vI = parseInt(v);
      }
  ...
  string: function(v) {
    ...
    if (typeof v != 'string') {
      if (this.strict) {
        return Modifiers.error({reason: 'not string', val: v});
      }
      else if (v != null) {
        v = v.toString();
      }
    ...
    var len = v.length;
    if (this.min != null && len < this.min) return Modifiers.error.call(this, {reason: 'min length', val: v});
    if (this.max != null && len > this.max) return Modifiers.error.call(this, {reason: 'max length', val: v});
    return v;
  },
  ...
  set: function(context) {
    if (typeof context != 'object') return Modifiers;
    var ctx = new Modifiers.empty();
...
if (typeof exports == 'object' && exports === this) {
  ...
  Modifiers.file = function(path) {
    if (!fs.statSync(path).isFile()) {
      return Modifiers.error({reason: 'not a file.', val : path});
    }
  ...
  Modifiers.dir = function(path) {
    if (!fs.statSync(path).isDirectory()) {
      return Modifiers.error({reason: 'not a directory.', val : path});
    }
    return (this.normalize) ? pth.normalize(path) : path;
  };
  ...
  Modifiers.path = function(path) {
    ...
    default: 
      if (!pth.existsSync(path)) {
        return Modifiers.error({reason: 'no such file or directory.', val : path});
      }
      return (this.normalize) ? pth.normalize(path) : path;
    case 'file':
...
function ModifiersTest() {
  ...
  try {
  console.log(Modifiers.set({max: 11}).integer(13));
  } catch (e) { console.log(e.message); }

  try {
  console.log(Modifiers.set({max: 11, strict: true}).integer("1"));
  } catch (e) { console.log(e.message); }
  ...
  try {
  console.log(Modifiers.integer.bind({max: 11, strict: true})(33));
  } catch (e) { console.log(e.message); }

  try {
  console.log(Modifiers.integer.bind({max: 11, strict: true, quiet: true})(33));
  } catch (e) { console.log(e.message); }
  ...
  console.log(Modifiers.set({pattern: /^get[A-Za-z0-9]+$/}).regex);
...
if (process.argv[1] === __filename) { ModifiersTest(); }
